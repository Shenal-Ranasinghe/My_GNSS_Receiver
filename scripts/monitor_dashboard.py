...
def find_latest_logdir():
    prefix = "/tmp/gnss-sdr-monitor-"
    dirs = [d for d in os.listdir("/tmp") if d.startswith("gnss-sdr-monitor-")]
    if not dirs:
        return None
...
def main():
    ...
    if logdir:
        ...
        print('No GNSS-SDR run directory provided; waiting for /tmp/gnss-sdr-monitor-* to appear...')
        try:
            while True:
                logdir = find_latest_logdir()
